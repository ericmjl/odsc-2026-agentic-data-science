# Agent instructions (odsc-agentic-data-science-2026)
...
- **CLI**: **Typer** — entrypoint wired in `pyproject.toml` under `[project.scripts]`; implementation lives under `odsc_agentic_data_science_2026/`.
...
- **Agent edits (`notebooks/*.py`)** — do **not** use `StrReplace`, patches, or direct file edits to Marimo notebook sources while iterating. The kernel owns the live graph. Follow **`.agents/skills/marimo-pair/`** and change cells only via **`marimo._code_mode`** (`execute-code.sh` + `async with marimo._code_mode.get_context() as ctx:` → `create_cell` / `edit_cell` / `delete_cell` / `run_cell`). Let the UI save back to disk when appropriate.
- **Imports** — use **one** `hide_code` cell at the top that imports shared libraries and **exports** the shared names the notebook needs via an explicit **`return mo, pathlib, pd, …`** (same pattern in the saved `.py`). **Do not** end the cell with only a parenthesized tuple; that is not the convention here. Downstream cells take those as dependencies; **do not** re-import the same modules under ad-hoc aliases (e.g. `_plt_hm`) to dodge multiply-defined-name errors.
- **No leading-underscore locals** — do not bind **`_foo`** names in cell bodies (temps, merges, paths, etc.). In Marimo they **break the reactive dependency graph**. Use **`merged`**, **`commit_tmpdir`**, **`adc_path`**, etc. For `for _ in ...` / `for _, row in ...`, use a real name (e.g. **`row_index`**) instead of **`_`**. (The generated `@app.cell def _():` wrapper in the `.py` file is Marimo boilerplate, not a cell-local temp.)
...
- **Package code**: `odsc_agentic_data_science_2026/` — main library and CLI.
